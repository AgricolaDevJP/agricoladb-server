// Code generated by ent, DO NOT EDIT.

package ent

import (
	"agricoladb/ent/card"
	"agricoladb/ent/cardspecialcolor"
	"agricoladb/ent/cardtype"
	"agricoladb/ent/deck"
	"agricoladb/ent/product"
	"agricoladb/ent/revision"
	"context"
	"encoding/json"
	"fmt"
	"sync"
	"sync/atomic"

	"entgo.io/contrib/entgql"
	"entgo.io/ent/dialect"
	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/schema"
	"github.com/99designs/gqlgen/graphql"
	"github.com/hashicorp/go-multierror"
	"golang.org/x/sync/semaphore"
)

// Noder wraps the basic Node method.
type Noder interface {
	Node(context.Context) (*Node, error)
}

// Node in the graph.
type Node struct {
	ID     int      `json:"id,omitempty"`     // node id.
	Type   string   `json:"type,omitempty"`   // node type.
	Fields []*Field `json:"fields,omitempty"` // node fields.
	Edges  []*Edge  `json:"edges,omitempty"`  // node edges.
}

// Field of a node.
type Field struct {
	Type  string `json:"type,omitempty"`  // field type.
	Name  string `json:"name,omitempty"`  // field name (as in struct).
	Value string `json:"value,omitempty"` // stringified value.
}

// Edges between two nodes.
type Edge struct {
	Type string `json:"type,omitempty"` // edge type.
	Name string `json:"name,omitempty"` // edge name.
	IDs  []int  `json:"ids,omitempty"`  // node ids (where this edge point to).
}

func (c *Card) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     c.ID,
		Type:   "Card",
		Fields: make([]*Field, 34),
		Edges:  make([]*Edge, 7),
	}
	var buf []byte
	if buf, err = json.Marshal(c.LiteralID); err != nil {
		return nil, err
	}
	node.Fields[0] = &Field{
		Type:  "string",
		Name:  "literal_id",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.RevisionID); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "int",
		Name:  "revision_id",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.PrintedID); err != nil {
		return nil, err
	}
	node.Fields[2] = &Field{
		Type:  "string",
		Name:  "printed_id",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.PlayAgricolaCardID); err != nil {
		return nil, err
	}
	node.Fields[3] = &Field{
		Type:  "string",
		Name:  "play_agricola_card_id",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.DeckID); err != nil {
		return nil, err
	}
	node.Fields[4] = &Field{
		Type:  "int",
		Name:  "deck_id",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.CardTypeID); err != nil {
		return nil, err
	}
	node.Fields[5] = &Field{
		Type:  "int",
		Name:  "card_type_id",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.CardSpecialColorID); err != nil {
		return nil, err
	}
	node.Fields[6] = &Field{
		Type:  "int",
		Name:  "card_special_color_id",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.NameJa); err != nil {
		return nil, err
	}
	node.Fields[7] = &Field{
		Type:  "string",
		Name:  "name_ja",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.NameEn); err != nil {
		return nil, err
	}
	node.Fields[8] = &Field{
		Type:  "string",
		Name:  "name_en",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.MinPlayersNumber); err != nil {
		return nil, err
	}
	node.Fields[9] = &Field{
		Type:  "int",
		Name:  "min_players_number",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.Prerequisite); err != nil {
		return nil, err
	}
	node.Fields[10] = &Field{
		Type:  "string",
		Name:  "prerequisite",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.Cost); err != nil {
		return nil, err
	}
	node.Fields[11] = &Field{
		Type:  "string",
		Name:  "cost",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.Description); err != nil {
		return nil, err
	}
	node.Fields[12] = &Field{
		Type:  "string",
		Name:  "description",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.Note); err != nil {
		return nil, err
	}
	node.Fields[13] = &Field{
		Type:  "string",
		Name:  "note",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.IsOfficialJa); err != nil {
		return nil, err
	}
	node.Fields[14] = &Field{
		Type:  "bool",
		Name:  "is_official_ja",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.VictoryPoint); err != nil {
		return nil, err
	}
	node.Fields[15] = &Field{
		Type:  "int",
		Name:  "victory_point",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.SpecialVictoryPoint); err != nil {
		return nil, err
	}
	node.Fields[16] = &Field{
		Type:  "string",
		Name:  "special_victory_point",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasArrrow); err != nil {
		return nil, err
	}
	node.Fields[17] = &Field{
		Type:  "bool",
		Name:  "has_arrrow",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasBonusPointIcon); err != nil {
		return nil, err
	}
	node.Fields[18] = &Field{
		Type:  "bool",
		Name:  "has_bonus_point_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasNegativeBonusPointIcon); err != nil {
		return nil, err
	}
	node.Fields[19] = &Field{
		Type:  "bool",
		Name:  "has_negative_bonus_point_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasPanIcon); err != nil {
		return nil, err
	}
	node.Fields[20] = &Field{
		Type:  "bool",
		Name:  "has_pan_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasBreadIcon); err != nil {
		return nil, err
	}
	node.Fields[21] = &Field{
		Type:  "bool",
		Name:  "has_bread_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasFarmPlannerIcon); err != nil {
		return nil, err
	}
	node.Fields[22] = &Field{
		Type:  "bool",
		Name:  "has_farm_planner_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasActionsBoosterIcon); err != nil {
		return nil, err
	}
	node.Fields[23] = &Field{
		Type:  "bool",
		Name:  "has_actions_booster_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasPointsProviderIcon); err != nil {
		return nil, err
	}
	node.Fields[24] = &Field{
		Type:  "bool",
		Name:  "has_points_provider_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasGoodsProviderIcon); err != nil {
		return nil, err
	}
	node.Fields[25] = &Field{
		Type:  "bool",
		Name:  "has_goods_provider_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasFoodProviderIcon); err != nil {
		return nil, err
	}
	node.Fields[26] = &Field{
		Type:  "bool",
		Name:  "has_food_provider_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasCropProviderIcon); err != nil {
		return nil, err
	}
	node.Fields[27] = &Field{
		Type:  "bool",
		Name:  "has_crop_provider_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasBuildingResourceProviderIcon); err != nil {
		return nil, err
	}
	node.Fields[28] = &Field{
		Type:  "bool",
		Name:  "has_building_resource_provider_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasLivestockProviderIcon); err != nil {
		return nil, err
	}
	node.Fields[29] = &Field{
		Type:  "bool",
		Name:  "has_livestock_provider_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasCutPeatIcon); err != nil {
		return nil, err
	}
	node.Fields[30] = &Field{
		Type:  "bool",
		Name:  "has_cut_peat_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasFellTreesIcon); err != nil {
		return nil, err
	}
	node.Fields[31] = &Field{
		Type:  "bool",
		Name:  "has_fell_trees_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasSlashAndBurnIcon); err != nil {
		return nil, err
	}
	node.Fields[32] = &Field{
		Type:  "bool",
		Name:  "has_slash_and_burn_icon",
		Value: string(buf),
	}
	if buf, err = json.Marshal(c.HasHiringFareIcon); err != nil {
		return nil, err
	}
	node.Fields[33] = &Field{
		Type:  "bool",
		Name:  "has_hiring_fare_icon",
		Value: string(buf),
	}
	node.Edges[0] = &Edge{
		Type: "Revision",
		Name: "revision",
	}
	err = c.QueryRevision().
		Select(revision.FieldID).
		Scan(ctx, &node.Edges[0].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[1] = &Edge{
		Type: "Product",
		Name: "products",
	}
	err = c.QueryProducts().
		Select(product.FieldID).
		Scan(ctx, &node.Edges[1].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[2] = &Edge{
		Type: "Deck",
		Name: "deck",
	}
	err = c.QueryDeck().
		Select(deck.FieldID).
		Scan(ctx, &node.Edges[2].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[3] = &Edge{
		Type: "CardType",
		Name: "card_type",
	}
	err = c.QueryCardType().
		Select(cardtype.FieldID).
		Scan(ctx, &node.Edges[3].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[4] = &Edge{
		Type: "CardSpecialColor",
		Name: "card_special_color",
	}
	err = c.QueryCardSpecialColor().
		Select(cardspecialcolor.FieldID).
		Scan(ctx, &node.Edges[4].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[5] = &Edge{
		Type: "Card",
		Name: "children",
	}
	err = c.QueryChildren().
		Select(card.FieldID).
		Scan(ctx, &node.Edges[5].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[6] = &Edge{
		Type: "Card",
		Name: "ancestors",
	}
	err = c.QueryAncestors().
		Select(card.FieldID).
		Scan(ctx, &node.Edges[6].IDs)
	if err != nil {
		return nil, err
	}
	return node, nil
}

func (csc *CardSpecialColor) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     csc.ID,
		Type:   "CardSpecialColor",
		Fields: make([]*Field, 3),
		Edges:  make([]*Edge, 1),
	}
	var buf []byte
	if buf, err = json.Marshal(csc.Key); err != nil {
		return nil, err
	}
	node.Fields[0] = &Field{
		Type:  "string",
		Name:  "key",
		Value: string(buf),
	}
	if buf, err = json.Marshal(csc.NameJa); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "string",
		Name:  "name_ja",
		Value: string(buf),
	}
	if buf, err = json.Marshal(csc.NameEn); err != nil {
		return nil, err
	}
	node.Fields[2] = &Field{
		Type:  "string",
		Name:  "name_en",
		Value: string(buf),
	}
	node.Edges[0] = &Edge{
		Type: "Card",
		Name: "cards",
	}
	err = csc.QueryCards().
		Select(card.FieldID).
		Scan(ctx, &node.Edges[0].IDs)
	if err != nil {
		return nil, err
	}
	return node, nil
}

func (ct *CardType) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     ct.ID,
		Type:   "CardType",
		Fields: make([]*Field, 3),
		Edges:  make([]*Edge, 1),
	}
	var buf []byte
	if buf, err = json.Marshal(ct.Key); err != nil {
		return nil, err
	}
	node.Fields[0] = &Field{
		Type:  "string",
		Name:  "key",
		Value: string(buf),
	}
	if buf, err = json.Marshal(ct.NameJa); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "string",
		Name:  "name_ja",
		Value: string(buf),
	}
	if buf, err = json.Marshal(ct.NameEn); err != nil {
		return nil, err
	}
	node.Fields[2] = &Field{
		Type:  "string",
		Name:  "name_en",
		Value: string(buf),
	}
	node.Edges[0] = &Edge{
		Type: "Card",
		Name: "cards",
	}
	err = ct.QueryCards().
		Select(card.FieldID).
		Scan(ctx, &node.Edges[0].IDs)
	if err != nil {
		return nil, err
	}
	return node, nil
}

func (d *Deck) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     d.ID,
		Type:   "Deck",
		Fields: make([]*Field, 4),
		Edges:  make([]*Edge, 2),
	}
	var buf []byte
	if buf, err = json.Marshal(d.Key); err != nil {
		return nil, err
	}
	node.Fields[0] = &Field{
		Type:  "string",
		Name:  "key",
		Value: string(buf),
	}
	if buf, err = json.Marshal(d.RevisionID); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "int",
		Name:  "revision_id",
		Value: string(buf),
	}
	if buf, err = json.Marshal(d.NameJa); err != nil {
		return nil, err
	}
	node.Fields[2] = &Field{
		Type:  "string",
		Name:  "name_ja",
		Value: string(buf),
	}
	if buf, err = json.Marshal(d.NameEn); err != nil {
		return nil, err
	}
	node.Fields[3] = &Field{
		Type:  "string",
		Name:  "name_en",
		Value: string(buf),
	}
	node.Edges[0] = &Edge{
		Type: "Card",
		Name: "cards",
	}
	err = d.QueryCards().
		Select(card.FieldID).
		Scan(ctx, &node.Edges[0].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[1] = &Edge{
		Type: "Revision",
		Name: "revision",
	}
	err = d.QueryRevision().
		Select(revision.FieldID).
		Scan(ctx, &node.Edges[1].IDs)
	if err != nil {
		return nil, err
	}
	return node, nil
}

func (pr *Product) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     pr.ID,
		Type:   "Product",
		Fields: make([]*Field, 5),
		Edges:  make([]*Edge, 2),
	}
	var buf []byte
	if buf, err = json.Marshal(pr.RevisionID); err != nil {
		return nil, err
	}
	node.Fields[0] = &Field{
		Type:  "int",
		Name:  "revision_id",
		Value: string(buf),
	}
	if buf, err = json.Marshal(pr.IsOfficialJa); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "bool",
		Name:  "is_official_ja",
		Value: string(buf),
	}
	if buf, err = json.Marshal(pr.NameJa); err != nil {
		return nil, err
	}
	node.Fields[2] = &Field{
		Type:  "string",
		Name:  "name_ja",
		Value: string(buf),
	}
	if buf, err = json.Marshal(pr.NameEn); err != nil {
		return nil, err
	}
	node.Fields[3] = &Field{
		Type:  "string",
		Name:  "name_en",
		Value: string(buf),
	}
	if buf, err = json.Marshal(pr.PublishedYear); err != nil {
		return nil, err
	}
	node.Fields[4] = &Field{
		Type:  "int",
		Name:  "published_year",
		Value: string(buf),
	}
	node.Edges[0] = &Edge{
		Type: "Card",
		Name: "cards",
	}
	err = pr.QueryCards().
		Select(card.FieldID).
		Scan(ctx, &node.Edges[0].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[1] = &Edge{
		Type: "Revision",
		Name: "revision",
	}
	err = pr.QueryRevision().
		Select(revision.FieldID).
		Scan(ctx, &node.Edges[1].IDs)
	if err != nil {
		return nil, err
	}
	return node, nil
}

func (r *Revision) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     r.ID,
		Type:   "Revision",
		Fields: make([]*Field, 3),
		Edges:  make([]*Edge, 3),
	}
	var buf []byte
	if buf, err = json.Marshal(r.Key); err != nil {
		return nil, err
	}
	node.Fields[0] = &Field{
		Type:  "string",
		Name:  "key",
		Value: string(buf),
	}
	if buf, err = json.Marshal(r.NameJa); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "string",
		Name:  "name_ja",
		Value: string(buf),
	}
	if buf, err = json.Marshal(r.NameEn); err != nil {
		return nil, err
	}
	node.Fields[2] = &Field{
		Type:  "string",
		Name:  "name_en",
		Value: string(buf),
	}
	node.Edges[0] = &Edge{
		Type: "Card",
		Name: "cards",
	}
	err = r.QueryCards().
		Select(card.FieldID).
		Scan(ctx, &node.Edges[0].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[1] = &Edge{
		Type: "Product",
		Name: "products",
	}
	err = r.QueryProducts().
		Select(product.FieldID).
		Scan(ctx, &node.Edges[1].IDs)
	if err != nil {
		return nil, err
	}
	node.Edges[2] = &Edge{
		Type: "Deck",
		Name: "decks",
	}
	err = r.QueryDecks().
		Select(deck.FieldID).
		Scan(ctx, &node.Edges[2].IDs)
	if err != nil {
		return nil, err
	}
	return node, nil
}

func (c *Client) Node(ctx context.Context, id int) (*Node, error) {
	n, err := c.Noder(ctx, id)
	if err != nil {
		return nil, err
	}
	return n.Node(ctx)
}

var errNodeInvalidID = &NotFoundError{"node"}

// NodeOption allows configuring the Noder execution using functional options.
type NodeOption func(*nodeOptions)

// WithNodeType sets the node Type resolver function (i.e. the table to query).
// If was not provided, the table will be derived from the universal-id
// configuration as described in: https://entgo.io/docs/migrate/#universal-ids.
func WithNodeType(f func(context.Context, int) (string, error)) NodeOption {
	return func(o *nodeOptions) {
		o.nodeType = f
	}
}

// WithFixedNodeType sets the Type of the node to a fixed value.
func WithFixedNodeType(t string) NodeOption {
	return WithNodeType(func(context.Context, int) (string, error) {
		return t, nil
	})
}

type nodeOptions struct {
	nodeType func(context.Context, int) (string, error)
}

func (c *Client) newNodeOpts(opts []NodeOption) *nodeOptions {
	nopts := &nodeOptions{}
	for _, opt := range opts {
		opt(nopts)
	}
	if nopts.nodeType == nil {
		nopts.nodeType = func(ctx context.Context, id int) (string, error) {
			return c.tables.nodeType(ctx, c.driver, id)
		}
	}
	return nopts
}

// Noder returns a Node by its id. If the NodeType was not provided, it will
// be derived from the id value according to the universal-id configuration.
//
//	c.Noder(ctx, id)
//	c.Noder(ctx, id, ent.WithNodeType(typeResolver))
func (c *Client) Noder(ctx context.Context, id int, opts ...NodeOption) (_ Noder, err error) {
	defer func() {
		if IsNotFound(err) {
			err = multierror.Append(err, entgql.ErrNodeNotFound(id))
		}
	}()
	table, err := c.newNodeOpts(opts).nodeType(ctx, id)
	if err != nil {
		return nil, err
	}
	return c.noder(ctx, table, id)
}

func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	switch table {
	case card.Table:
		query := c.Card.Query().
			Where(card.ID(id))
		query, err := query.CollectFields(ctx, "Card")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
		if err != nil {
			return nil, err
		}
		return n, nil
	case cardspecialcolor.Table:
		query := c.CardSpecialColor.Query().
			Where(cardspecialcolor.ID(id))
		query, err := query.CollectFields(ctx, "CardSpecialColor")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
		if err != nil {
			return nil, err
		}
		return n, nil
	case cardtype.Table:
		query := c.CardType.Query().
			Where(cardtype.ID(id))
		query, err := query.CollectFields(ctx, "CardType")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
		if err != nil {
			return nil, err
		}
		return n, nil
	case deck.Table:
		query := c.Deck.Query().
			Where(deck.ID(id))
		query, err := query.CollectFields(ctx, "Deck")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
		if err != nil {
			return nil, err
		}
		return n, nil
	case product.Table:
		query := c.Product.Query().
			Where(product.ID(id))
		query, err := query.CollectFields(ctx, "Product")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
		if err != nil {
			return nil, err
		}
		return n, nil
	case revision.Table:
		query := c.Revision.Query().
			Where(revision.ID(id))
		query, err := query.CollectFields(ctx, "Revision")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
		if err != nil {
			return nil, err
		}
		return n, nil
	default:
		return nil, fmt.Errorf("cannot resolve noder from table %q: %w", table, errNodeInvalidID)
	}
}

func (c *Client) Noders(ctx context.Context, ids []int, opts ...NodeOption) ([]Noder, error) {
	switch len(ids) {
	case 1:
		noder, err := c.Noder(ctx, ids[0], opts...)
		if err != nil {
			return nil, err
		}
		return []Noder{noder}, nil
	case 0:
		return []Noder{}, nil
	}

	noders := make([]Noder, len(ids))
	errors := make([]error, len(ids))
	tables := make(map[string][]int)
	id2idx := make(map[int][]int, len(ids))
	nopts := c.newNodeOpts(opts)
	for i, id := range ids {
		table, err := nopts.nodeType(ctx, id)
		if err != nil {
			errors[i] = err
			continue
		}
		tables[table] = append(tables[table], id)
		id2idx[id] = append(id2idx[id], i)
	}

	for table, ids := range tables {
		nodes, err := c.noders(ctx, table, ids)
		if err != nil {
			for _, id := range ids {
				for _, idx := range id2idx[id] {
					errors[idx] = err
				}
			}
		} else {
			for i, id := range ids {
				for _, idx := range id2idx[id] {
					noders[idx] = nodes[i]
				}
			}
		}
	}

	for i, id := range ids {
		if errors[i] == nil {
			if noders[i] != nil {
				continue
			}
			errors[i] = entgql.ErrNodeNotFound(id)
		} else if IsNotFound(errors[i]) {
			errors[i] = multierror.Append(errors[i], entgql.ErrNodeNotFound(id))
		}
		ctx := graphql.WithPathContext(ctx,
			graphql.NewPathWithIndex(i),
		)
		graphql.AddError(ctx, errors[i])
	}
	return noders, nil
}

func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	noders := make([]Noder, len(ids))
	idmap := make(map[int][]*Noder, len(ids))
	for i, id := range ids {
		idmap[id] = append(idmap[id], &noders[i])
	}
	switch table {
	case card.Table:
		query := c.Card.Query().
			Where(card.IDIn(ids...))
		query, err := query.CollectFields(ctx, "Card")
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, node := range nodes {
			for _, noder := range idmap[node.ID] {
				*noder = node
			}
		}
	case cardspecialcolor.Table:
		query := c.CardSpecialColor.Query().
			Where(cardspecialcolor.IDIn(ids...))
		query, err := query.CollectFields(ctx, "CardSpecialColor")
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, node := range nodes {
			for _, noder := range idmap[node.ID] {
				*noder = node
			}
		}
	case cardtype.Table:
		query := c.CardType.Query().
			Where(cardtype.IDIn(ids...))
		query, err := query.CollectFields(ctx, "CardType")
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, node := range nodes {
			for _, noder := range idmap[node.ID] {
				*noder = node
			}
		}
	case deck.Table:
		query := c.Deck.Query().
			Where(deck.IDIn(ids...))
		query, err := query.CollectFields(ctx, "Deck")
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, node := range nodes {
			for _, noder := range idmap[node.ID] {
				*noder = node
			}
		}
	case product.Table:
		query := c.Product.Query().
			Where(product.IDIn(ids...))
		query, err := query.CollectFields(ctx, "Product")
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, node := range nodes {
			for _, noder := range idmap[node.ID] {
				*noder = node
			}
		}
	case revision.Table:
		query := c.Revision.Query().
			Where(revision.IDIn(ids...))
		query, err := query.CollectFields(ctx, "Revision")
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, node := range nodes {
			for _, noder := range idmap[node.ID] {
				*noder = node
			}
		}
	default:
		return nil, fmt.Errorf("cannot resolve noders from table %q: %w", table, errNodeInvalidID)
	}
	return noders, nil
}

type tables struct {
	once  sync.Once
	sem   *semaphore.Weighted
	value atomic.Value
}

func (t *tables) nodeType(ctx context.Context, drv dialect.Driver, id int) (string, error) {
	tables, err := t.Load(ctx, drv)
	if err != nil {
		return "", err
	}
	idx := int(id / (1<<32 - 1))
	if idx < 0 || idx >= len(tables) {
		return "", fmt.Errorf("cannot resolve table from id %v: %w", id, errNodeInvalidID)
	}
	return tables[idx], nil
}

func (t *tables) Load(ctx context.Context, drv dialect.Driver) ([]string, error) {
	if tables := t.value.Load(); tables != nil {
		return tables.([]string), nil
	}
	t.once.Do(func() { t.sem = semaphore.NewWeighted(1) })
	if err := t.sem.Acquire(ctx, 1); err != nil {
		return nil, err
	}
	defer t.sem.Release(1)
	if tables := t.value.Load(); tables != nil {
		return tables.([]string), nil
	}
	tables, err := t.load(ctx, drv)
	if err == nil {
		t.value.Store(tables)
	}
	return tables, err
}

func (*tables) load(ctx context.Context, drv dialect.Driver) ([]string, error) {
	rows := &sql.Rows{}
	query, args := sql.Dialect(drv.Dialect()).
		Select("type").
		From(sql.Table(schema.TypeTable)).
		OrderBy(sql.Asc("id")).
		Query()
	if err := drv.Query(ctx, query, args, rows); err != nil {
		return nil, err
	}
	defer rows.Close()
	var tables []string
	return tables, sql.ScanSlice(rows, &tables)
}
