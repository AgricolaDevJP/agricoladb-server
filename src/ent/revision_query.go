...
type RevisionQuery struct {
	...
	loadTotal         []func(context.Context, []*Revision) error
	withNamedCards    map[string]*CardQuery
	withNamedProducts map[string]*ProductQuery
	withNamedDecks    map[string]*DeckQuery
...
func (rq *RevisionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Revision, error) {
	...
	if query := rq.withCards; query != nil {
		if err := rq.loadCards(ctx, query, nodes,
			func(n *Revision) { n.Edges.Cards = []*Card{} },
			func(n *Revision, e *Card) { n.Edges.Cards = append(n.Edges.Cards, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withProducts; query != nil {
		if err := rq.loadProducts(ctx, query, nodes,
			func(n *Revision) { n.Edges.Products = []*Product{} },
			func(n *Revision, e *Product) { n.Edges.Products = append(n.Edges.Products, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withDecks; query != nil {
		if err := rq.loadDecks(ctx, query, nodes,
			func(n *Revision) { n.Edges.Decks = []*Deck{} },
			func(n *Revision, e *Deck) { n.Edges.Decks = append(n.Edges.Decks, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range rq.withNamedCards {
		if err := rq.loadCards(ctx, query, nodes,
			func(n *Revision) { n.appendNamedCards(name) },
			func(n *Revision, e *Card) { n.appendNamedCards(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range rq.withNamedProducts {
		if err := rq.loadProducts(ctx, query, nodes,
			func(n *Revision) { n.appendNamedProducts(name) },
			func(n *Revision, e *Product) { n.appendNamedProducts(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range rq.withNamedDecks {
		if err := rq.loadDecks(ctx, query, nodes,
			func(n *Revision) { n.appendNamedDecks(name) },
			func(n *Revision, e *Deck) { n.appendNamedDecks(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RevisionQuery) loadCards(ctx context.Context, query *CardQuery, nodes []*Revision, init func(*Revision), assign func(*Revision, *Card)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Revision)
	for i := range nodes {
...
func (rq *RevisionQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Revision, init func(*Revision), assign func(*Revision, *Product)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Revision)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (rq *RevisionQuery) loadDecks(ctx context.Context, query *DeckQuery, nodes []*Revision, init func(*Revision), assign func(*Revision, *Deck)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Revision)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (rq *RevisionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(revision.Table, revision.Columns, sqlgraph.NewFieldSpec(revision.FieldID, field.TypeInt))
	_spec.From = rq.sql
...
func (rq *RevisionQuery) WithNamedCards(name string, opts ...func(*CardQuery)) *RevisionQuery {
	...
	if rq.withNamedCards == nil {
		rq.withNamedCards = make(map[string]*CardQuery)
	}
	rq.withNamedCards[name] = query
	return rq
}
...
func (rq *RevisionQuery) WithNamedProducts(name string, opts ...func(*ProductQuery)) *RevisionQuery {
	...
	if rq.withNamedProducts == nil {
		rq.withNamedProducts = make(map[string]*ProductQuery)
	}
	rq.withNamedProducts[name] = query
	return rq
}
...
func (rq *RevisionQuery) WithNamedDecks(name string, opts ...func(*DeckQuery)) *RevisionQuery {
	...
	if rq.withNamedDecks == nil {
		rq.withNamedDecks = make(map[string]*DeckQuery)
	}
	rq.withNamedDecks[name] = query
	return rq
}
